...
Then, let's calculate manually the projection of the first customer (row one) into the first *principal component* (PC1) by multiplying these vectors we get the projection.
```{r}
