---
title: "Supervised ML Validation Protocol"
...
Let's split the *data_train* into K=5 folds of equal size and store them in a *list* with K=5 *data.frames*.
```{r}
K <- 5
...
This project will use the *AUC* as performance metric to choose the best parametrization of each *supervised ML* algorithm. Let's define the function *AUC* manually although we could use implemented libraries.
```{r}
